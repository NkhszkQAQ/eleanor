...
在 ../../content.js 的 photos 数组增删路径即可调整相册，默认每批最多追加 24 张；照片墙优先加载 responsive/ 下按尺寸选择的 AVIF，兼容回退至 previews/ 下的 WebP，点击后查看未改动原文件。纯静态网页不会自动扫描文件夹。alt、caption 只填写用户确认的描述，目前留空。width、height 用于预留正确比例，须考虑 EXIF 方向。
...
新增 previewAvif 字段及格式／尺寸说明见 responsive/README.md；点击查看器先复用预览，原图完成后替换。
