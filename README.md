...
- `audio.song.title` 可填写歌名；留空显示“音乐”。音频路径在各自 `src` 修改。`audio.song.volume` 控制默认音量（0–1，当前 0.2）；`audio.song.playAfterOpening: true` 表示主动拆开礼物后开始播放，设为 false 可恢复手动播放。
...
音乐为 `assets/audio/song.mp3` 独立文件（原始 MP3 未转码，3,645,565 字节），由 GitHub Pages 托管，不嵌入 HTML／JS。所有音频默认 `preload="none"`；首次拆包或主动播放时才请求音乐，点开信封才请求录音，照片按需懒加载。浏览器缓存可复用已请求素材。未知时长显示 `--:--`，取得真实媒体元数据后再更新。
