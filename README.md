...
- 用文本编辑器打开 `content.js`。`hero.title` 当前为网站名称 Eleanor，`hero.body` 留空；清空两项可隐藏首页标题区，任意一项填写后只显示该项。
...
首页首次打开时展示可轻触／点击的拆包裹开场，约 1.45 秒后进入首页；同一标签页会话中刷新或返回首页不会重复。可随时跳过或按 Esc 进入，首页底部可“重播开场”。键盘操作和系统“减少动态效果”直接进入，不播放展开动画。`content.js` 的 `opening.enabled` 可关闭开场，相关提示在 `labels` 配置。此开场不自动播放音乐；若浏览器禁止会话存储，开场仍可关闭，但下次进入可能再次展示。
